...
class Jetson {
  public:
    ...
    Jetson(const struct device *uartdev);

    // Init Jetson with a SPISlave Object
    // Jetson(SPISlave &SPIJetson);

    Jetson(const struct spi_dt_spec *spidev);
  ...
  private:
    ...
    // const struct device *jetsonUart;
    SerialBase jetsonSerial; 
    


    const struct spi_dt_spec *jetsonSPI;    
...
class RefWritePacket : public WritePacket {
  ...
  private:
    void get_packed_payload(Jetson::WriteState &write_state,
        ...
        memcpy(&buff[0], &write_state.game_state, sizeof(uint8_t));
        memcpy(&buff[1], &write_state.team_color, sizeof(uint8_t));
        memcpy(&buff[2], &write_state.robot_hp, sizeof(uint16_t));
    }
};
...
class RobotStateWritePacket : public WritePacket {
  ...
  private:
    void get_packed_payload(Jetson::WriteState &write_state,
        ...
        memcpy(&buff[0], &write_state.yaw_angle_rads, sizeof(float));
        memcpy(&buff[4], &write_state.pitch_angle_rads, sizeof(float));

        memcpy(&buff[8], &write_state.yaw_velocity, sizeof(float));
        memcpy(&buff[12], &write_state.pitch_velocity, sizeof(float));
                              
        memcpy(&buff[16], &write_state.chassis_x_velocity, sizeof(float));
        memcpy(&buff[20], &write_state.chassis_y_velocity, sizeof(float));
        memcpy(&buff[24], &write_state.chassis_rotation, sizeof(float));
    }
};
...
class EmbeddedUserInputWritePacket : public WritePacket {
  ...
  private:
    void get_packed_payload(Jetson::WriteState &write_state,
        ...
        memcpy(&buff[0], &write_state.calibration, sizeof(uint8_t));
        memcpy(&buff[1], &write_state.activate_CV, sizeof(uint8_t));
    }
};
...
class ChassisReadPacket : public ReadPacket {
  ...
  private:
    void extract_payload(char *buff, Jetson::ReadState &read_state) override {
        ...
        // calibration byte
        memcpy(&read_state.desired_x_vel, &buff[0], sizeof(float));
        memcpy(&read_state.desired_y_vel, &buff[4], sizeof(float));
